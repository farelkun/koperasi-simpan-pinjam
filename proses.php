...
if ($_POST['reqa'] == "add")
{
	...
	else
	{
		...
		$tambah_anggota = mysql_query($sql_anggota) or die(mysql_error());
		$tambah_simpanan = mysql_query($sql_simpanan) or die(mysql_error());
...
else if ($_POST['reqa'] == "edit")
{
	...
	else
	{
		...
		$sql = mysql_query("UPDATE anggota set nama='".$nama."',alamat='".$alamat."',tgl_lahir='".$tgl_lahir."',tmp_lahir='".$tmp_lahir."',j_kel='".$j_kel."',no_telp='".$no_telp."' WHERE id_anggota='".$id_anggota."'");
...
if ($_GET['reqa'] == "dell")
{
	...
	$sql_pin = mysql_query("SELECT * FROM pinjaman WHERE id_anggota='".$id_anggota."'");
	$data_pin = mysql_fetch_array($sql_pin);
	$jumlah_pin = mysql_num_rows($sql_pin);
...
if ($_POST['reqs'] == "add")
{
	...
	$sql = mysql_query("SELECT * FROM simpanan WHERE bln='".$bln."' AND id_anggota='".$id_anggota."' AND nm_simpanan='Simpanan Wajib'");
	...
	else if($nm_simpanan == "Simpanan Sukarela")
	{
		...
		$tambah_simpanan = mysql_query($sql);
...
if ($_GET['reqs'] == "dell")
{
	...
	mysql_query("DELETE FROM simpanan where id_simpanan='".$id_simpanan."'");
	header('Location:page-simpanan.php');
	exit;
}
...
if ($_POST['reqpin'] == "add")
{
	...
	$bln = date('m');
	$thn = date('Y');
	$sql = mysql_query("SELECT * FROM pinjaman WHERE id_anggota='".$id_anggota."' AND ket='0'");
	$jum = mysql_num_rows($sql);
	...
	else if ($jum == 0)
	{
		...
		$tambah = mysql_query($mysql) or die(mysql_error());
...
if ($_GET['reqpin'] == 'dell')
{
	...
	$delete = mysql_query("DELETE FROM pinjaman WHERE id_pinjaman='".$id_pinjaman."'");
...
if ($_POST['reqang'] == "add")
{
	...
	$jum_angsuran_ke = mysql_query("SELECT * FROM angsuran WHERE bln='".$bln_p."' AND id_anggota='".$id_anggota."'");;
	$num = mysql_num_rows($jum_angsuran_ke);
	...
	if ($num == 0)
	{
		...
		$tambah_denda = mysql_query($sqldenda);
...
if ($_GET['reqang'] == 'dell')
{
	...
	$delete = mysql_query("DELETE FROM angsuran WHERE id_angsuran='".$id_angsuran."'");
